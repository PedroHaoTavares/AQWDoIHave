...
function httpGet(theUrl, nodeList, arrayOffset, x, isMonster, isQuest, isMerge)
{
	...
	function checkData() {
		...
		if (xmlHttp.readyState == 4) {
			...
			xmlHttp = null; // Clear the reference instead of delete
...
async function addLocationIcon(nodeList, nodeText, Items, arrayOffset, x, isList, isMerge) {
	// CRITICAL FIX: Correctly match items from Wiki against Inventory
	// Items from ProcessAccountItems have various normalizations applied
	// nodeText from Wiki has suffixes removed and is lowercase
	
	// Normalize search text: lowercase and trim
	const searchText = nodeText.toLowerCase().trim();
	
	let itemIndex = -1;
	
	// Strategy 1: Direct case-insensitive match
	for (let i = 0; i < Items.length; i++) {
		const storedItem = String(Items[i]).toLowerCase().trim();
		if (storedItem === searchText) {
			itemIndex = i;
			break;
		}
	}
	
	// Strategy 2: If not found, try matching with wiki suffixes removed
	// (in case Items still has them while nodeText doesn't)
	if (itemIndex === -1 && typeof wiki_exclude_suffixes !== 'undefined') {
		let cleanedSearchText = searchText;
		for (let suffix of wiki_exclude_suffixes["Excluded"] || []) {
			cleanedSearchText = cleanedSearchText.replace(suffix, "").trim();
		}
		
		for (let i = 0; i < Items.length; i++) {
			let storedItem = String(Items[i]).toLowerCase().trim();
			// Also clean the stored item
			for (let suffix of wiki_exclude_suffixes["Excluded"] || []) {
				storedItem = storedItem.replace(suffix, "").trim();
			}
			if (storedItem === cleanedSearchText) {
				itemIndex = i;
				break;
			}
		}
	...
	// If still not found, item is not in inventory - don't show icon
	if (itemIndex === -1) {
		return;
	}
	
	// Validate DOM node exists and is valid
	const currentNode = nodeList[arrayOffset + x];
	if (!currentNode || typeof currentNode.appendChild !== 'function' || !currentNode.parentNode) {
		return;
	}
	
	// Get the item location from the Where array
	const itemLocation = Where[itemIndex];
	
	// Only proceed if we have a valid location
	if (!itemLocation || (itemLocation !== "Bank" && itemLocation !== "Inventory")) {
		return;
	}
	
	// Create a BRAND NEW icon element for this specific item
	// This prevents reusing the same element across multiple items
	const where_icon = document.createElement("a");
	where_icon.style.cursor = "pointer";
	where_icon.style.marginLeft = "5px";
	where_icon.style.display = "inline-block";
	
	// Set icon based on location
	if (itemLocation === "Bank") {
		where_icon.innerHTML = "<img title='In Bank' style='height:20px; vertical-align:middle;' src='" + bank_icon + "' />";
	} else {
		// Inventory
		where_icon.innerHTML = "<img title='In Inventory' style='height:20px; vertical-align:middle;' src='" + inv_icon + "' />";
	}
	
	// FINAL STEP: Append icon to correct DOM location
	// Each item gets its icon appended exactly once, to its own line
	try {
		if (isList) {
			// List context: append to parent container
			currentNode.parentNode.appendChild(where_icon);
		} else if (isMerge || window.location.href === 'http://aqwwiki.wikidot.com/new-releases') {
			// Merge/new releases: append inside the link element
			currentNode.appendChild(where_icon);
		} else {
			// Regular wiki pages: append to parent to keep on same line
			currentNode.parentNode.appendChild(where_icon);
		}
	} catch (error) {
		console.error("Failed to append location icon for: " + searchText, error);
	}
}
